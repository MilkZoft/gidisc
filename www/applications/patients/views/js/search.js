...
$("#inputsearch_patients").click( function() {
	alert("aa");
	if($.trim($("input[name='search_patients']").val()) != "") {
		alert("xxx");
		var post = { "search" : $("input[name='search_patients']").val()};

		$("#videos").html('');
		search(post, "/search/ajax/patients");
	}
});
...
function getObjectUser(data) {
	...
	var object  = '<tr>';
		...
		object = object + '<td>' + data["Name"]    + '</td>';
		object = object + '<td>' + data["Surname"] + '</td>';
