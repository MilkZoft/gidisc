...
function getTable($caption, $thead, $tFoot, $total, $comments = FALSE, $app = FALSE) {
					...
					if($tFoot) {
						foreach($tFoot as $column) {                    	
							...
							if(isset($column["Name"])) { 
								$HTML .= '<td class="center">'. $column["Name"] .'</td>';
							}
							
							if(isset($column["Center"])) {
								$href =  path(whichApplication() . _sh . "cpanel" . _sh . "show" . _sh . $column["Center"]["ID_Center"]);
								$HTML .= '<td class="center"><a href="'. $href .'">'. $column["Center"]["Name"] .'</a></td>';
							}
							...
							if(isset($column["Address"])) { 
								$HTML .= '<td class="center">'. $column["Address"] .'</td>';
							}
							
							if(isset($column["Phone"])) { 
								$HTML .= '<td class="center">'. $column["Phone"] .'</td>';
							}
	...
	$HTML .= '	<div class="table-options" style="position: relative; z-index: 1; margin-bottom: 25px;">
					&nbsp;'. __("Select") .': 
					...
					if(segment(3) === "trash" or segment(4) === "trash") { 
	...
	if($data) {		
		foreach($data as $record) {			
			...
			if($application === "ads") {
				...
				$tFoot = $CPanel_Model->getTFoot($a, "Situation", 	$record["Situation"], 	  		     $tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "Action",      $action, 			      	  		 $tFoot);	
			} elseif($application === "centers") {
				if($record["Situation"] === "Deleted") {
					$action = $CPanel_Model->action(TRUE, $record["ID_Center"]);
				} else {
					$action = $CPanel_Model->action(FALSE, $record["ID_Center"]);
				}
				
				$tFoot = $CPanel_Model->getTFoot($a, "checkbox",  getCheckbox($record["ID_Center"]), 	$tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "i", 		   $color, 					  			$tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "ID", 		   $record["ID_Center"], 	      	    $tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "Center", array("Name" => $record["Name"], "ID_Center" => $record["ID_Center"]), $tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "Address",    $record["Address"], 					$tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "Phone",      $record["Phone"], 					$tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "Situation",  $record["Situation"], 	  			$tFoot);
				$tFoot = $CPanel_Model->getTFoot($a, "Action",     $action, 			      	  		$tFoot);
			}
		...
		if($application !== "comments") {
			if($trash) {
				showAlert("No results", path($application ."/cpanel/results"));
			} else {
				redirect($application ."/cpanel/add");
			}
		} else {
			return FALSE;
		}
...
function getFields($application) {
	if($application === "ads") {
		...
		return "ID, Name, Address, Phone, Situation";
	}
...
function getSearch() {
	...
	$HTML  = formOpen(path(segment(1) . _sh . "cpanel" . _sh . "results"), "form-results-search");
	$HTML .= br();
